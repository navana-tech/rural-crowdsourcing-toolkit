...
constructor(
  ...
  suspend fun getTaskInfo(): List<TaskInfo> = withContext(Dispatchers.IO) {
      ...
      val taskInfoList = tasks.map { task ->
          ...
          val completed = assignmentTasks.count { assignment -> assignment.status == MicrotaskAssignmentStatus.COMPLETED }
          val submitted = assignmentTasks.count { assignment -> assignment.status == MicrotaskAssignmentStatus.SUBMITTED }
